class SolutionDijkstra:
    def minimumEffortPath(self, heights: List[List[int]]) -> int:
        ...
        while queue:
            ...
            for (ro, co) in [(0, -1), (-1, 0), (0, 1), (1, 0)]: # left, up, right, down
                next_row, next_col = row + ro, col + co
                if 0 <= next_row < rows and 0 <= next_col < cols \
                    and not visited[next_row][next_col]:

                    new_effort = abs(heights[next_row][next_col] - heights[row][col])

                    new_effort = max(new_effort, distances[row][col])

                    if new_effort < distances[next_row][next_col]:
                        distances[next_row][next_col] = new_effort

                        heapq.heappush(queue, (new_effort, next_row, next_col))
